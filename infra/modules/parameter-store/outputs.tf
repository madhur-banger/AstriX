...
output "kms_key_arn" {
  description = <<-EOT
    ARN of the KMS key the SecureString parameters are actually encrypted
    with. null means the AWS-managed alias/aws/ssm key is in use, which has no
    stable ARN to scope IAM policies to.
  EOT
  value       = local.parameter_kms_key_id
}
