...
> **Note on this revision:** AstriX's backend was migrated off MongoDB/Mongoose onto PostgreSQL (via Drizzle ORM) and Redis (via `ioredis`) — see [`backend/migrations/PLAN.md`](../../backend/migrations/PLAN.md) for the full six-phase history. Rate limiting used to be backed by `rate-limit-mongo` (a custom `MongoStore`, shared across every limiter, falling back to an in-memory store if `MONGO_URI` was unset); it's now backed by `rate-limit-redis`'s `RedisStore` unconditionally, since Redis is a hard dependency of the app regardless. The Express app assembly itself also moved: what used to live directly in `index.ts` now lives in a separate `backend/src/app.ts` (a pure `buildApp(): Express` factory with no `listen()` call), so tests can build a real, fully-wired app without booting an actual server — `index.ts` now only owns startup/shutdown. This chapter documents the pipeline as it exists today and calls out prior Mongo-era behavior only as explicit history, never as how the system behaves now.
...
Orthogonal to the algorithm is *where the counters live*. An **in-memory** store (the default for `express-rate-limit` if no store is configured) keeps counters in the Node process's own memory — trivial to set up, but only correct if there's exactly one process handling all traffic. The moment an app runs as multiple instances behind a load balancer, each instance has its own independent counter, and the *effective* limit becomes (configured limit) × (instance count) — plus every deploy or instance replacement silently resets every counter to zero. A **shared-store** backend — Redis (via `rate-limit-redis` or `rate-limiter-flexible`'s Redis adapter, the most common choice at scale because `INCR` is a single atomic Redis primitive built for exactly this) or a relational database — centralizes the counters so every instance is enforcing the same real budget. As of this revision, AstriX uses exactly this: a Redis-backed shared counter, covered in full below.

## 2. AstriX's Choice

AstriX uses Express's linear, ordered middleware chain — no onion model, no decorator layer. Global, cross-cutting middleware (security headers, structured logging, body parsing, cookie parsing, CORS, and a baseline rate limiter) is registered exactly once in `backend/src/app.ts`, in a fixed order, and applies to every request that reaches the app. On top of that floor, route-specific middleware — the `authenticate` JWT guard and a set of purpose-built rate limiters — is layered on per-router or per-route, closest to the handlers it protects. Rate limiting itself is a **fixed-window, Redis-backed shared counter**, built on `express-rate-limit` with `rate-limit-redis`'s `RedisStore`, chosen specifically so the limit is enforced consistently across every running instance of the backend rather than per-process.

## 3. AstriX Implementation

### 3.1 The rate-limiter factory

Every limiter in the app — the global floor and every route-specific one — is built by a single factory function:

```ts
// backend/src/utils/rate-limiter.ts, in full
import rateLimit, {
  type Options,
...
This is deliberately simpler than the pre-migration version: there's no fallback-to-in-memory branch, no lazily-constructed shared client, no hand-written `incr`/`decrement`/`resetKey` adapter. `redis` (`backend/src/redis/client.ts`) is a single shared `ioredis` instance the whole app already depends on for sessions and tokens, so `RedisStore` is constructed unconditionally on every `createRateLimiter()` call, passing it `redis.call(...)` as its low-level command executor and a `prefix` unique to that limiter — `rate-limit-redis` handles the atomic-increment-plus-TTL mechanics internally rather than requiring a hand-rolled key-namespacing adapter the way the old Mongo store did. `standardHeaders: true` / `legacyHeaders: false` means every limited response carries the modern `RateLimit-*` headers (limit, remaining, reset) rather than the deprecated `X-RateLimit-*` set, which is the current IETF-draft-aligned convention `express-rate-limit` ships.
...
300 requests per 15-minute window, per client IP, namespaced under the `ratelimit:api:` key prefix in Redis — completely independent of any per-route limiter's counter, even though every limiter shares the same underlying `redis` client instance.
...
`00-master-backend-architecture.md` reproduces `app.ts` in full; the excerpt that matters specifically for *pipeline ordering* — from app creation through the last piece of global middleware before routes are mounted — is this, read in sequence exactly as Express executes it:

```ts
// backend/src/app.ts (excerpt, inside buildApp())
const app = express();
...
**Why Redis for rate limiting.** AstriX's backend runs as multiple ECS Fargate tasks behind an Application Load Balancer — there is no single process handling all traffic, so an in-memory rate-limit store (the `express-rate-limit` default) would give each task its own independent counter. A `max: 5` login limiter would, in practice, allow up to `5 × (number of running tasks)` attempts, and every deploy or task replacement — which happens routinely — would silently reset every counter back to zero, handing an attacker a fresh budget for free. A shared, cluster-wide store is not optional here; it's the only way the configured numbers mean what they say. Redis is purpose-built for exactly this access pattern — an atomic `INCR` primitive and native per-key `EXPIRE` — and it's already a hard dependency of the app for sessions and single-use tokens (see [`02-authentication-and-authorization.md`](./02-authentication-and-authorization.md) §3.2), so backing rate limiting with it too is "extend an existing dependency," not "add a new piece of infrastructure." This is a genuine improvement over the pre-migration design, which backed rate limiting with MongoDB specifically because Redis wasn't yet a dependency of the app at all — that constraint no longer exists.

**Why every limiter gets its own `RedisStore` instance rather than sharing one.** `createRateLimiter()` constructs a fresh `RedisStore` on every call, all pointed at the same underlying `redis` client (`backend/src/redis/client.ts`) but each given a unique `prefix`. Because the actual TCP connection is shared (one `ioredis` instance for the whole process, reused everywhere Redis is touched — sessions, tokens, cache, rate limiting), there's no per-limiter connection cost to economize on the way the old Mongo-backed design had to with its single memoized `MongoStore`; the only thing that has to stay disciplined is the `prefix`, which is what keeps two limiters' counters from colliding under the same physical Redis keyspace.

**Why global middleware is registered once in `app.ts` rather than per-router.** Helmet, structured logging, body parsing, cookie parsing, CORS, and the floor rate limiter apply identically to every request the app serves, regardless of which domain router eventually handles it. Registering each of those six pieces once, before any router is mounted, means there is exactly one place to look to know what every request goes through, and — more importantly — no way for a newly added router to accidentally skip one of them by omission. The alternative (each router individually importing and re-mounting `helmet()`, `cors()`, etc.) would work, but would make "is this route actually CORS-protected" a question that requires checking every router file individually rather than one glance at `app.ts`. Route-specific concerns (which endpoints need auth, which need a stricter rate limit) are the opposite case — they vary per route, so they're deliberately *not* forced into the same one-size-fits-all global layer, and instead sit closest to the specific route they modify.
...
Rate limiting is squarely current practice now, too. Fixed-window counting is the simplest correct algorithm for the problem, and it's what AstriX uses via `express-rate-limit`'s default strategy — that's a reasonable, well-tested choice, but it is the *oldest* of the three algorithms surveyed in §1, and it does carry the boundary-burst characteristic described there. A token-bucket or sliding-window approach would tolerate legitimate bursty traffic more gracefully without loosening the real abuse ceiling, and libraries like `rate-limiter-flexible` support those strategies as first-class options on Node today. The store, though, is exactly the 2026 default assumption at real scale: a Redis-backed distributed limiter, purpose-built for this access pattern (atomic increment, native TTL, microsecond-scale latency, no interaction with the app's primary transactional workload). The remaining gap is the algorithm, not the store — worth revisiting if login/API traffic grew by an order of magnitude and the boundary-burst characteristic of fixed-window counting became a real problem in practice, not a theoretical one.
