...
| **API** | REST, layered (routes → middleware → controllers → services → Drizzle/Postgres), Zod validation, centralized error handling, Swagger (non-prod only), Redis-backed rate limiting | [`backend/00`](./backend/00-master-backend-architecture.md)–[09](./backend/09-api-design-and-external-providers.md) |
...
| **Testing** | Backend: 21 files, unit+integration+E2E, real Postgres/Redis via `testcontainers`, 90/85/90/90 coverage gate in CI. Frontend: 22 files, component/hook only — **no E2E (Playwright/Cypress), no coverage gate** | [`testing/00`](./testing/00-master-testing-strategy.md)–[08](./testing/08-security-testing-and-scanning.md) |
...
| **Search** | Typing a task title into a search box is the #1 navigation pattern in Jira/Linear once a workspace has more than ~30 tasks. | Start cheap: Postgres **full-text search** (`tsvector`/`tsquery` + a GIN index on `tasks.title`/`description` and `projects.name`) — zero new infra, expressed directly in the existing Drizzle service layer. `pg_trgm` is the fallback if fuzzy/typo-tolerant substring matching turns out to matter more than ranked full-text. Do **not** stand up Elasticsearch/Meilisearch until a real workspace's task count and query patterns justify it (ranking, typo tolerance, faceted filters) — that's a Phase 3+ decision, revisit with actual usage data. |
...
| **Caching layer (Redis)** | Redis is already live infra (sessions, single-use tokens, rate limiting all run on it post-cutover — see [`backend/migrations/`](../backend/migrations/PLAN.md)), so this item is now "extend an existing dependency," not "stand up a new one." The remaining real use: hot, low-churn reads — workspace metadata, role/permission lookups — currently hit Postgres on every request and would benefit from a cache-aside layer (the `cacheAside` helper already exists in `services/redis/cache.ts`, just not yet wired into those read paths). | AWS ElastiCache for Redis (new `infra/modules/redis` — a security group opening 6379 from ECS only, per the existing `infra/modules/security` scaffolding that already documents this port) once this moves beyond local Docker Compose. At AstriX's current scale, Upstash (serverless Redis, pay-per-request) is a legitimate cheaper alternative to a standing ElastiCache node — decide based on expected request volume, not by default. |
...
| **Database migrations** | `drizzle-kit` already generates and applies real, ordered SQL migrations (`src/db/migrations/`) — this item is closed, not a gap | Keep it that way as new tables (`Comment`/`Attachment`/`Notification`) land: every schema change goes through `drizzle-kit generate` + a committed migration file, never a hand-edited table |
...
| **Backup/DR** | Postgres backup story depends on how it's hosted (RDS automated backups + snapshots, or a self-managed `pg_dump`/WAL-archiving setup) — not yet decided or documented for this repo; no documented RTO/RPO or restore runbook either | Decide the hosting/backup mechanism explicitly (don't default to "whatever the instance type includes"), then write the restore runbook before it's needed under pressure, not during an incident |
...
| Self-hosted search (Elasticsearch cluster) | Defer | Postgres full-text search first (Phase 1); revisit with real usage data, not speculatively — an Elasticsearch cluster is real ops burden for a team of this size. |
...
2. Backend Phase 1, in this order: comments → attachments (+ S3 bucket) → activity log → notifications (still synchronous) → Postgres full-text search. Each is independently shippable and user-visible.
